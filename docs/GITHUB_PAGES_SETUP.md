...
1. Push this repository to  
   ...
   **Recommended:** run `npm install` at the **repo root**, then **commit `package-lock.json`**. CI uses **`npm ci`** for reproducible installs. If the lockfile is missing from git, update the workflow to use `npm install` instead.

   **Optional speed-up:** after the lockfile is in git, you can add npm caching to `setup-node` in `.github/workflows/deploy-pages.yml` (see comments in that file).
...
4. Push to **`main`** or **`master`**.  
   The workflow **Deploy to GitHub Pages** (in **`.github/workflows/deploy-pages.yml`** at the **repository root**) runs **`npm ci`** and **`npm run build`** at the root, then uploads **`dist/`**.
...
This workspace uses a **single-root** Vite app:
...
website-trial-v1/          ← Git repo root
  ...
  src/
  scripts/                 ← optional helpers (e.g. build-and-push.sh)
```
