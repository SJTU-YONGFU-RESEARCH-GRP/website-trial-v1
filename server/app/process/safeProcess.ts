...
export class SafeProcessRunner {
  ...
  constructor(private readonly cancelGraceMs = 10_000) {}
  ...
  async run(key: string, options: ProcessRunOptions): Promise<ProcessRunResult> {
    ...
    if (options.tool.interpreterPath && entryPoint) entryPoint = await fsp.realpath(path.isAbsolute(entryPoint) ? entryPoint : path.join(options.tool.rootPath || "", entryPoint));
    const adapterArgv = options.tool.interpreterPath && entryPoint ? [entryPoint, ...options.argv] : options.argv;
    if (!adapterArgv.every((item) => typeof item === "string" && !item.includes("\0"))) throw new Error("argv contains an invalid value");
    const environment: NodeJS.ProcessEnv = { PATH: process.env.PATH, LANG: "C.UTF-8", LC_ALL: "C.UTF-8" };
    ...
    const handleOutput = (stream: "stdout" | "stderr", chunk: Buffer) => {
      ...
      if (outputBytes > (options.maxOutputBytes || 256 * 1024 * 1024)) { outputLimitExceeded = true; void this.terminate(child); return; }
  ...
  private async terminate(child: ChildProcess): Promise<void> {
    ...
    await new Promise<void>((resolvePromise) => {
      const timer = setTimeout(() => { signalProcessGroup(child.pid!, "SIGKILL"); resolvePromise(); }, this.cancelGraceMs);
