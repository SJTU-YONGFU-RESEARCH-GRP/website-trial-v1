...
/** Quantities that can drive Sankey link width (architecture / bit width are node topology). */
export type SankeyFlowValueKey = keyof Pick<AdderDemoRow, "fmaxMhz" | "powerMw" | "areaUm2">;

const SANKEY_POOL_METRIC_LABEL: Record<SankeyFlowValueKey, string> = {
  powerMw: "power (mW)",
  areaUm2: "area (µm²)",
  fmaxMhz: "Fmax (MHz)",
};
...
/**
 * Each design (architecture × bit width) links to a 32b or 64b aggregate pool;
 * link thickness = chosen PPA scalar on that row.
 */
export function plotlySankeyByBitwidth(valueKey: SankeyFlowValueKey): {
  labels: string[];
  source: number[];
  target: number[];
  value: number[];
  ...
  const pool32 = n;
  const pool64 = n + 1;
  const metricLabel = SANKEY_POOL_METRIC_LABEL[valueKey];
  const labels = [
    ...rows.map((r) => `${formatArchLabel(r.architecture)} ${r.bitWidth}b`),
    `32-bit pool (Σ ${metricLabel})`,
    `64-bit pool (Σ ${metricLabel})`,
  ];
  const source: number[] = [];
  const target: number[] = [];
  const value: number[] = [];
