...
export function ResultsPanel(): JSX.Element {
  ...
  const load = useCallback(async () => {
    try {
      const [nextResults, nextUsers] = await Promise.all([listAdminResults({ moduleId: moduleId || undefined, lifecycle: lifecycle || undefined, limit: 200 }), listAdminUsers()]);
  ...
  const visible = ownerId ? results.filter((result) => result.ownerId === ownerId) : results;
