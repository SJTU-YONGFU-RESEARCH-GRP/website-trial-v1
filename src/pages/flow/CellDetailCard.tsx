...
export function CellDetailCard({
  cellResult,
  algorithms,
}: CellDetailCardProps): JSX.Element {
  if (!cellResult) {
    return (
  ...
  return (
    <div className="chart-card">
      <h2>Cell Detail: <code>{cellResult.cellName}</code></h2>
      <p className="hint">
        Side-by-side comparison of layout, parasitic, and timing data across algorithms.
      </p>
        ...
        <table className="analog-table" style={{ minWidth: "600px" }}>
          <thead>
            <tr>
              <th>Metric</th>
              {algorithms.map((a) => (
                <th key={a.algorithmId}><code>{a.algorithmName}</code></th>
              ))}
            </tr>
          </thead>
          <tbody>
            <tr>
              ...
              {algorithms.map((a) => {
                ...
                return (
                  <td key={a.algorithmId}>
                    <Badge status={ar?.status ?? "pending"} />
                  </td>
                );
              })}
            </tr>
            <tr>
              ...
              {algorithms.map((a) => {
                const ar = cellResult.algorithmResults.find((r) => r.algorithmId === a.algorithmId);
                return <td key={a.algorithmId}>{ar?.layout?.widthNm ?? "—"}</td>;
              })}
            </tr>
            <tr>
              <td>Area (µm²)</td>
              {algorithms.map((a) => {
                const ar = cellResult.algorithmResults.find((r) => r.algorithmId === a.algorithmId);
                return <td key={a.algorithmId}>{ar?.layout?.areaUm2.toFixed(1) ?? "—"}</td>;
              })}
            </tr>
            <tr>
              <td>Utilization</td>
            ...
            <tr>
              ...
              {algorithms.map((a) => {
                const ar = cellResult.algorithmResults.find((r) => r.algorithmId === a.algorithmId);
                return <td key={a.algorithmId}>{ar?.timing?.geomeanDelayPs.toFixed(1) ?? "—"}</td>;
              })}
            </tr>
            <tr>
              <td>Avg Net Cap (fF)</td>
              {algorithms.map((a) => {
                const ar = cellResult.algorithmResults.find((r) => r.algorithmId === a.algorithmId);
                const avg = ar?.parasitics
                  ? +(ar.parasitics.netCaps.reduce((s, n) => s + n.cTotalFF, 0) / ar.parasitics.netCaps.length).toFixed(2)
                  : null;
            ...
            <tr>
              ...
              {algorithms.map((a) => {
                const ar = cellResult.algorithmResults.find((r) => r.algorithmId === a.algorithmId);
                return <td key={a.algorithmId}>{ar?.layout?.runtimeSec.toFixed(1) ?? "—"}</td>;
              })}
            </tr>
            <tr>
          ...
          {algorithms.map((a) => {
            ...
            return (
              <div key={a.algorithmId} style={{ flex: "1 1 200px" }}>
                <strong style={{ fontSize: "0.75rem" }}>{a.algorithmName}</strong>
                <div
                  className="flow-layout-preview__svg"
                  style={{ marginTop: "0.25rem" }}
                  dangerouslySetInnerHTML={{ __html: ar.layout.layoutPreviewSvg }}
                />
              </div>
            );
          })}
